...
class PPO(Algorithm):
    ...
    def __init__(
        ...
        self.masks = torch.zeros(
            (self.num_steps, self.num_envs, self.env.action_space.n), dtype=torch.bool
        ).to(self.device)
    ...
    def learn(self, total_timesteps: int = 500000):
        ...
        next_masks = torch.tensor(
            np.array(self.env.env_method("action_masks")), dtype=torch.bool
        ).to(self.device)
        ...
        for update in range(1, num_updates + 1):
            ...
            with torch.no_grad():
                ...
                for t in reversed(range(self.num_steps)):
                    ...
                    delta = (
                        self.rewards[t] + self.gamma * nextvalues * nextnonterminal - self.values[t]
                    )
            ...
            self.writer.add_scalar(
                "charts/FPS", int(global_step / (time.time() - start_time)), global_step
            )
