...
SEED = 42
MONITOR_PATH = f"./results_2obj/ppo_deepset_{SEED}"
...
for i, weights in tqdm(enumerate(reward_weights)):
    ...
    model_path = f"./results/ppo_deepset_{SEED}_{weights[0]}_{weights[1]}.zip"
    ...
    # Warm start from the previous weights
    if i > 0:
        agent.load(
            f"./results_2obj/ppo_deepset_{SEED}_{reward_weights[i - 1][0]}_{reward_weights[i - 1][1]}.zip"
        )
    agent.learn(total_timesteps=1000000)
    agent.save(f"./results_2obj/ppo_deepset_{SEED}_{weights[0]}_{weights[1]}.zip")
...
# %%
# eval
agent_files = [f for f in os.listdir("./results_2obj") if f.endswith(".zip")]
...
for file in tqdm(agent_files):
    ...
    for _ in range(n_episodes):
        ...
        while not done:
            ...
            action_mask = np.array(env.env_method("action_masks"))
            vec_rew = [info[0]["block_reward"], info[0]["energy_reward"]]
            ep_vec_rew.append(vec_rew)
        ep_vec_rewards.append(np.sum(ep_vec_rew, axis=0))
    vec_rewards.append(np.mean(ep_vec_rewards, axis=0))

# %%
np.savetxt(f"./results_csv/ppo_deepset_{SEED}_rewards_2obj.csv", vec_rewards, delimiter=",")
